...
static void joy_digital(int jnum, uint32_t mask, uint32_t code, char press, int bnum, int dont_save = 0)
{
	...
	if (num < NUMPLAYERS)
	{
		...
		if (user_io_osd_is_visible() || (bnum == BTN_OSD))
		{
			...
			{
				osdbtn |= mask;
				if ((mask & (JOY_BTN1 | JOY_BTN2)) && (osdbtn & (JOY_BTN1 | JOY_BTN2)) == (JOY_BTN1 | JOY_BTN2))
				{
					osdbtn |= JOY_BTN3;
					mask = JOY_BTN3;
				}
...
static void input_cb(struct input_event *ev, struct input_absinfo *absinfo, int dev)
{
	...
	bool not_text_mapping = !(mapping && mapping_type == 3 && mapping_button);
	int cancel   = ev->type == EV_KEY && ev->code == KEY_ESC   && not_text_mapping;
	int enter    = ev->type == EV_KEY && ev->code == KEY_ENTER && not_text_mapping;
		...
		&& input[dev].quirk != QUIRK_MSSP)
	{
		...
		bool assigning_kbd_toggle = is_menu() && mapping < 2 && mapping_button == SYS_BTN_OSD_KTGL;
		if (ev->type == EV_KEY && !assigning_kbd_toggle && !map_skip)
		{
